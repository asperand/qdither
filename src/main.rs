...
fn main() {
    ...
    let user_palette = match user_palette_result { // Handling for no file found.
        ...
        Err(_) =>  get_colors(&mut image_tuple.0,*palette_colors),
    };
...
fn eu_distance_pixels(current_pixel:RGB<u8>, compared_pixel:RGB<u8>) -> f32 {
    let eu_distance = 
            (((current_pixel.r as f32 - compared_pixel.r as f32)).powi(2)
            +((current_pixel.g as f32 - compared_pixel.g as f32)).powi(2)
            +((current_pixel.b as f32 - compared_pixel.b as f32)).powi(2))
            .sqrt();
    return eu_distance;
}
...
fn get_colors(image:&mut Vec<RGB<u8>>, palette_colors:u8) -> Vec<RGB<u8>> {
   ...
   loop {
        ...
        for i in 0..(new_cent_vec.len()){
            if eu_distance_pixels(new_cent_vec[i],prev_cent_vec[i]) > 16.0 { // TODO: custom tolerance?
                is_converged = false;
            }
            else{
                is_converged = true;
